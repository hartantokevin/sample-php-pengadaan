		...
		<div class="card card-info">
			...
			<div class="card-body">
				<table class="table table-bordered table-striped">
					...
					<?php
						include 'koneksi.php';
						$data = mysqli_query($koneksi, "SELECT * FROM permintaan join paket join kontrak WHERE permintaan.no=paket.no AND permintaan.no=kontrak.no");
						while($d = mysqli_fetch_array($data))
						{
							...
							$status_pemeriksaan = mysqli_query($koneksi, "SELECT * FROM pemeriksaan WHERE no='".$d['no']."'");
							if(mysqli_num_rows($status_pemeriksaan) == 1)
								echo "<td class='align-middle'><i class='fas fa-check'></i></td>";
							else
								echo "<td class='align-middle'><i class='fas fa-times'></i></td>";
							
							$status_pembayaran = mysqli_query($koneksi, "SELECT * FROM pembayaran WHERE no='".$d['no']."'");
							if(mysqli_num_rows($status_pembayaran) == 1)
								echo "<td class='align-middle'><i class='fas fa-check'></i></td>";
							else
								echo "<td class='align-middle'><i class='fas fa-times'></i></td>";
							echo "</tr>";
