...
<div class="wrapper">
  ...
  <div class="content-wrapper">
	...
	<?php
		include 'koneksi.php';
		$data = mysqli_query($koneksi, "SELECT * FROM paket WHERE no='".$_GET['no']."'");
			...
			<div class="card-body">
				...
				<div class="form-group">
					...
					<div class="input-group">
						...
						<input type="text" class="form-control" name="nama_paket" value="<?php echo $col['nama_paket']; ?>">
					</div>
				</div>
				<div class="form-group">
					<label>Akun</label>
						...
						<select class="form-control" name="akun">
							<?php
								...
								while($akun = mysqli_fetch_array($dataakun))
									if($akun['akun'] == $col['akun'])
										echo "<option selected>".$akun['akun']."</option>";
									else
										echo "<option>".$akun['akun']."</option>";
							?>
						</select>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="input-group">
						...
						<input type="text" class="form-control" name="nilai_hps" value="<?php echo $col['nilai_hps']; ?>">
					</div>
				</div>
				<div>
					<button type="submit" class="btn btn-info" style="background-color: #17a2b8" name="submit">Update</button>
					<button class="btn btn-default"><a href="kontrak.php">Batal</a></button>
                </div>
			</div>
			</form>
			<?php
				if(isset($_POST['submit']))
				{
					...
					mysqli_query($koneksi, "UPDATE paket SET nama_paket='$nama_paket', akun='$akun', nilai_hps='$nilai_hps' WHERE no='$no'");
					echo "<script>window.location = 'kontrak.php';</script>";
				}
			?>
		</div>
      </div><!-- /.container-fluid -->
